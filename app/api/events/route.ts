...
export async function GET() {
  try {
    // Redis에서 이벤트 데이터 가져오기
    let events = []

    try {
      const { safeGetRedisData, parseRedisData } = await import("@/lib/redis")
      const eventsData = await safeGetRedisData("events")
      events = parseRedisData(eventsData) || []
    } catch (error) {
      console.error("이벤트 데이터 가져오기 오류:", error)
      events = []
    }
    ...
    return NextResponse.json({ error: "이벤트를 가져오는 중 오류가 발생했습니다" }, { status: 500 })
  }
}
